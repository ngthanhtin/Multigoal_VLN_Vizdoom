...
class ThreeGoals_GroundingEnv:
    ...
    def reset(self):
        ...
        self.time = 0
        self.num_goals = 3
        self.is_reach = [False, False, False]
        self.reach_priority = [1,1,1]

        #-----------------------------------------------------
        self.instruction, self.connection_type_list, instruction_id = self.get_random_instruction()
        
        seperate_instructions = self.instruction.split(self.connection_type_list[0], maxsplit=1)
        self.instruction_1 = seperate_instructions[0][:-1]
        seperate_instructions = seperate_instructions[1][1:].split(self.connection_type_list[1], maxsplit=1)
        self.instruction_2 = seperate_instructions[0][:-1]
        self.instruction_3 = seperate_instructions[1][1:]

        #set priority
        if self.connection_type_list[0] == 'and' and self.connection_type_list[1] == 'and':
            self.reach_priority = [1, 1, 1]
        if self.connection_type_list[0] == 'after' and self.connection_type_list[1] == 'and':
            self.reach_priority = [2, 1, 1]
        if self.connection_type_list[0] == 'before' and self.connection_type_list[1] == 'and':
            self.reach_priority = [1, 2, 2]
        if self.connection_type_list[0] == 'and' and self.connection_type_list[1] == 'after':
            self.reach_priority = [2, 2, 1]
        if self.connection_type_list[0] == 'and' and self.connection_type_list[1] == 'before':
            self.reach_priority = [1, 1, 2]

        # Retrieve the possible correct objects for the instruction. (3 goals)
        ...
        third_correct_location = np.random.randint(5)
        while third_correct_location == self.correct_location_list[0] or third_correct_location == self.correct_location_list[1]:
            third_correct_location = np.random.randint(5)
        self.correct_location_list.append(third_correct_location)


        # Randomly select 3 correct objects from the
        ...
        correct_object_id_3 = np.random.choice(correct_objects_list[2])
        i = 0
        while correct_object_id_2 == correct_object_id_1:
            correct_object_id_2 = np.random.choice(correct_objects_list[1])
        while correct_object_id_3 == correct_object_id_1 or correct_object_id_3 == correct_object_id_2:
            correct_object_id_3 = np.random.choice(correct_objects_list[2])
        ...
        chosen_correct_object_3 = [x for x in self.objects_info if
                                 x.name == self.objects[correct_object_id_3]][0]
        ...
        if 'largest' not in self.instruction_1 and 'largest' not in self.instruction_2 \
                ...
                object_ids = random.sample([x for x in range(len(self.objects))
                                        if x not in correct_objects_list[0] and x not in correct_objects_list[1] and x not in correct_objects_list[2]], 2)
        else:
            if ('largest' in self.instruction_1 or 'smallest' in self.instruction_1) and ('largest' in self.instruction_2 or 'smallest' in self.instruction_2):
                object_ids = self.get_candidate_objects_superlative_instr(
                                            chosen_correct_object_1, [chosen_correct_object_2, chosen_correct_object_3], self.instruction_1)[:2] #return 4 but just take only 2
                ...
                object_ids = self.get_candidate_objects_superlative_instr(
                                            chosen_correct_object_2, [chosen_correct_object_1, chosen_correct_object_3], self.instruction_2)[:2] #return 4 but just take only 2
                object_ids = [self.object_dict[x] for x in object_ids]                             
        ...
        object_ids.insert(self.correct_location_list[2], correct_object_id_3)
    ...
    def step(self, action_id):
        ...
        is_final = False
        if self.time == self.params.max_episode_length or reward == WRONG_OBJECT_REWARD or sum(self.is_reach) == self.num_goals:
            is_final = True
        else:
            is_final = False
        ...
        return state, reward, is_final, self.is_reach # None
    ...
    def get_reward(self):
        ...
        target_location_3 = self.object_coordinates[self.correct_location_list[2]]
        ...
        dist_3 = get_l2_distance(self.agent_x, self.agent_y,
                               target_location_3.x, target_location_3.y)
        total_reward = 0.
        shaping_reward = 0.
        #

        if dist_1 <= REWARD_THRESHOLD_DISTANCE:
            if self.is_reach[0] == True:
                total_reward += WRONG_OBJECT_REWARD
            else:
                if self.reach_priority == [1,1,1]:
                    # and, and
                    self.is_reach[0] = True
                    total_reward += CORRECT_OBJECT_REWARD
                if self.reach_priority == [1,2,2]:
                    # before, and
                    self.is_reach[0] = True
                    total_reward += CORRECT_OBJECT_REWARD
                if self.reach_priority == [2,1,1]:
                    # after, and
                    if self.is_reach[1] + self.is_reach[2] == 2:
                        self.is_reach[0] = True
                        total_reward += CORRECT_OBJECT_REWARD
                    else:
                        total_reward += WRONG_OBJECT_REWARD
                if self.reach_priority == [2,2,1]:
                    # and, after
                    if self.is_reach[2] == False:
                        total_reward += WRONG_OBJECT_REWARD
                    else:
                        self.is_reach[0] = True
                        total_reward += CORRECT_OBJECT_REWARD
                if self.reach_priority == [1,1,2]:
                    # and, before
                    self.is_reach[0] = True
                    total_reward += CORRECT_OBJECT_REWARD

        if dist_2 <= REWARD_THRESHOLD_DISTANCE:
            if self.is_reach[1] == True:
                total_reward += WRONG_OBJECT_REWARD
            else:
                if self.reach_priority == [1,1,1]:
                    # and, and
                    self.is_reach[1] = True
                    total_reward += CORRECT_OBJECT_REWARD
                if self.reach_priority == [1,2,2]:
                    # before, and
                    if self.is_reach[0] == False:
                        total_reward += WRONG_OBJECT_REWARD
                    else:
                        self.is_reach[1] = True
                        total_reward += CORRECT_OBJECT_REWARD
        ...
        if dist_3 <= REWARD_THRESHOLD_DISTANCE:
            if self.is_reach[2] == True:
                total_reward += WRONG_OBJECT_REWARD
            else:
                if self.reach_priority == [1,1,1]:
                    # and, and
                    self.is_reach[2] = True
                    total_reward += CORRECT_OBJECT_REWARD
                if self.reach_priority == [1,2,2]:
                    # before, and
                    if self.is_reach[0] == False:
                ...
                if self.reach_priority == [1,1,2]:
                    # and, before
                    if self.is_reach[0] + self.is_reach[1] == 2:
                        self.is_reach[2] = True
                        total_reward += CORRECT_OBJECT_REWARD
                    else:
                        total_reward += WRONG_OBJECT_REWARD

        if dist_1 > REWARD_THRESHOLD_DISTANCE and dist_2 > REWARD_THRESHOLD_DISTANCE and dist_3 > REWARD_THRESHOLD_DISTANCE:
            for i, object_location in enumerate(self.object_coordinates):
                if i == self.correct_location_list[0] or i == self.correct_location_list[1] or i == self.correct_location_list[2]:
                    continue
    ...
    def get_candidate_objects_superlative_instr(self, correct_object, avoid_objects, instruction):
        '''
    ...
    def get_random_instruction(self):
        ...
        connection_type_list = self.instructions[instruction_id]['connection_type']
        return instruction, connection_type_list, instruction_id
