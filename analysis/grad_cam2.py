...
import argparse
from env.multigoal_env import MultiGoal_GroundingEnv
from env.threegoals_env import ThreeGoals_GroundingEnv
...
def GradCAM():
    ...
    model.load_state_dict(torch.load('/home/tinvn/TIN/NLP_RL_Code/FFT_VLN_Vizdoom/saved/train_convolve_failed', map_location=torch.device(device)))
    ...
    while True:
        ...
        if len(fft_images_feats_arr) == 1000 and dft == True:
            ...
            fft_images_feats_arr = []
            fft_text_feats_arr = []
            result.to_pickle('../new_convolve_failed.df')
            exit()
        ##
